...
export default function About() {
  ...
  const competedHackathons = hackathonItems.filter(
    (hackathon) => !hackathon.name.toLowerCase().includes("mentor")
  );
  const filteredHackathonWinners = competedHackathons.filter(
    (hackathon) => hackathon.awards.length > 0
  );
  const filteredHackathonMentorship = hackathonItems.filter((hackathon) =>
    hackathon.name.toLowerCase().includes("mentor")
  );
