...
export default function NavigationGroupEntry({
  section,
}: {
  ...
  return (
    <div>
      ...
      >
        <FontAwesomeIcon
          icon={faChevronUp}
          className={`chevron-icon chevron-${chevronState}`}
        />
      ...
      {section.info && section.info.length > 0 ? (
        <div
          className={`navigation-entry-info-container ${chevronState == "right" ? "visible" : "invisible"}`}
        >
          <div className="navigation-entry-info-text">{section.info}</div>
        </div>
      ) : null}
    </div>
  );
